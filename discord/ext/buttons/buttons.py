# Original source at https://github.com/EvieePy/buttons
import asyncio
import discord
import inspect
...
class Paginator(Session):
    ...
    author:
        Only available when embed=True. The author to set for the embeded pages.
    author_url:
        Only available when embed=True. The author icon url to set for the embeded pages.
    footer:
        Only available when embed=True. The footer to set for the embeded pages.
    footer_url:
        Only available when embed=True. The footer icon url to set for the embeded pages.
    """
    ...
    def __init__(self, *, title: str = '', length: int = 10, entries: list = None,
                 ...
                 joiner: str = '\n', timeout: int = 180, thumbnail: str = None, author: str = None,
                 author_url: str = None, footer: str = None, footer_url: str = None
                 ):
        super().__init__()
    ...
    async def _paginate(self, ctx: commands.Context):
        ...
        for chunk in entries:
            if not self.use_embed:
                ...
                if self.footer and not self.footer_url:
                    embed.set_footer(text=self.footer)
                if self.footer_url and not self.footer:
                    embed.set_footer(icon_url=self.footer_url)
                if self.footer and self.footer_url:
                    embed.set_footer(text=self.footer, icon_url=self.footer_url)

                if self.author and not self.author_url:
